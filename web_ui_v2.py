...
if agent:
    ...
    twijfel_count = len(agent.training_examples.get('twijfel_nee', []))
    nee_count = len(agent.training_examples.get('nee', []))
    total_count = ja_count + twijfel_count + nee_count
    
    if total_count > 0:
        st.markdown(f"""
        <div class="training-badge">
            🎓 Getraind met {total_count} voorbeelden 
            (✅ {ja_count} JA | ◐ {twijfel_count} TWIJFEL | ✗ {nee_count} NEE)
        </div>
        """, unsafe_allow_html=True)
    else:
...
with st.sidebar:
    ...
    **Categorieën:**
    - ✅ Definitief (≥75 punten)
    - 🟡 Waarschijnlijk (60-74)
    - ⚠️ Twijfelgeval (45-59)
    - ❌ Geen natuurhuisje (<45)
    """)
    ...
    if agent and total_count > 0:
        ...
        st.metric("TWIJFEL voorbeelden", twijfel_count)
        ...
        with st.spinner("🤖 AI aan het werk (met training data)..."):
            try:
                ...
                if result.similar_to:
                    ...
                    similarity_label = {
                        'ja': 'JA voorbeelden',
                        'twijfel_nee': 'TWIJFEL voorbeelden',
                        'nee': 'NEE voorbeelden'
                    }
                ...
                with col2:
                    if st.button("❓ Twijfelachtig", use_container_width=True):
                        st.info("Noteer deze URL om aan training data toe te voegen.")
