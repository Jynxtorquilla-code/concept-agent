...
"""
            ...
            if ja_examples:
                prompt += "═══ JA - DEFINITIEF NATUURHUISJE (75-100 punten) ═══\n\n"
...
"""
            ...
            # Add TWIJFEL examples
            twijfel_examples = self.training_examples.get('twijfel_nee', [])[:2]
            if twijfel_examples:
                prompt += "═══ TWIJFEL/NEE - NIET ECHT (45-60 punten) ═══\n\n"
                for ex in twijfel_examples:
                    prompt += f"""◐ Voorbeeld (Score: {ex.score}/100):
   Redenering: {ex.reasoning}
   Kenmerken: {', '.join(ex.key_features[:3])}
...
"""
            ...
            if nee_examples:
                prompt += "═══ NEE - GEEN NATUURHUISJE (0-45 punten) ═══\n\n"
...
{{
    ...
    "similar_to": "<'ja', 'twijfel_nee', of 'nee' - welke categorie lijkt het meest op>",
...
"""
    ...
    def _calculate_final_score(self, analysis: Dict) -> ScoringResult:
        ...
        if total_score >= 75:
            ...
            category = "Waarschijnlijk natuurhuisje"
            is_natuurhuisje = True
        elif total_score >= 45:
            category = "Twijfelgeval"
